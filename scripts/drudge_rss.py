...
rssFileName = 'd:/work/lymanepp.github.io/drudge.rss'
jsonFileName = 'c:/Users/Lyman/AppData/Local/drudge.json'
...
def get_description(link):
	try:
		...
		if response.status_code == 200:
			...
			if description:
				return description.attrs['content'].strip().replace('\r', ' ').replace('\n', ' ').replace('  ', ' ')
	except:
		#print(link)
		#print(sys.exc_info())
		pass
...
for added, link, dict in index:
	...
	itemDesc.text = ('<b>' + urlparse(link).netloc + '</b><br>' + dict['description']).strip()
